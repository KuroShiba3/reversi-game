...
@pytest.mark.parametrize(
    "initial_setup,place_pos,disc,expected_flips,description",
    [
        # 上方向 (UP)
        (
            {
                Position(3, 3): Disc.WHITE,
                Position(2, 3): Disc.WHITE,
                Position(1, 3): Disc.WHITE,
            },
            Position(0, 3),
            Disc.BLACK,
        ...
        (
            {
                Position(3, 3): Disc.WHITE,
                Position(4, 3): Disc.WHITE,
                Position(5, 3): Disc.WHITE,
            },
            Position(6, 3),
            Disc.BLACK,
        ...
        (
            {
                Position(3, 3): Disc.WHITE,
                Position(3, 2): Disc.WHITE,
                Position(3, 1): Disc.WHITE,
            },
            Position(3, 0),
            Disc.BLACK,
        ...
        (
            {
                Position(3, 3): Disc.WHITE,
                Position(2, 2): Disc.WHITE,
                Position(1, 1): Disc.WHITE,
            },
            Position(0, 0),
            Disc.BLACK,
        ...
        (
            {
                Position(3, 3): Disc.WHITE,
                Position(2, 4): Disc.WHITE,
                Position(1, 5): Disc.WHITE,
            },
            Position(0, 6),
            Disc.BLACK,
        ...
        (
            {
                Position(3, 3): Disc.WHITE,
                Position(4, 2): Disc.WHITE,
                Position(5, 1): Disc.WHITE,
            },
            Position(6, 0),
            Disc.BLACK,
        ...
        (
            {
                Position(3, 3): Disc.WHITE,
                Position(4, 4): Disc.WHITE,
                Position(5, 5): Disc.WHITE,
            },
            Position(6, 6),
            Disc.BLACK,
...
def test_board_place_disc_flip_all_directions(
    initial_setup, place_pos, disc, expected_flips, description
):
    """8方向すべての裏返しをテスト"""
    # 空のボードを作成
    ...
    for pos in expected_flips:
        assert board.cells[pos] == disc, (
            f"{description}: 位置{pos}は{disc}であるべきです"
        )


@pytest.mark.parametrize(
    "board_setup,disc,expected_positions,description",
    [
        # ゲーム開始直後の黒の有効手
        (
            "initial",
...
def test_get_valid_moves(board_setup, disc, expected_positions, description):
    ...
    # ソートして比較（順序は問わない）
    assert sorted(valid_moves, key=lambda p: (p.row, p.col)) == sorted(
        expected_positions, key=lambda p: (p.row, p.col)
    ), f"{description}: 期待される有効手と一致しません"


@pytest.mark.parametrize(
    "board_setup,place_pos,disc,error_message,description",
    [
        # すでに石がある位置に置こうとする
        (
            "initial",
...
def test_place_disc_invalid_position(
    board_setup, place_pos, disc, error_message, description
):
